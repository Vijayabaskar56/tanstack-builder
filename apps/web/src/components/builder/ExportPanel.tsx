...
export function ExportPanel({ fields, onImport }: ExportPanelProps) {
  ...
  const generateFormCode = () => {
    ...
    const fieldComponents = fields.map(field => {
      ...
      const placeholder = field.appearance?.placeholder ? `placeholder="${field.appearance.placeholder}"` : ''
      const required = field.validation?.required ? 'required' : ''
      const helpText = field.appearance?.helpText ? `helpText="${field.appearance.helpText}"` : ''
      ...
      switch (field.type) {
        ...
        case 'radio': {
          ...
          const options = radioField.options?.map((opt: Option) => `    { label: "${opt.label}", value: "${opt.value}" }`).join(',\n') || ''
          return `  <RadioGroup ${baseProps} ${label} ${required}>\n    options={[\n${options}\n    ]}\n  />`
        }
        case 'select': {
          const selectField = field as Field & { options: Option[] }
          const selectOptions = selectField.options?.map((opt: Option) => `    { label: "${opt.label}", value: "${opt.value}" }`).join(',\n') || ''
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Button onClick={copyInstallCommand} variant="outline" size="sm" className="w-full">
            {copied ? (
              <>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Generated Form Code</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <Button onClick={copyFormCode} variant="outline" size="sm" className="w-full">
