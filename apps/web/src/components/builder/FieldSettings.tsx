...
import type { Field, Option, ChoiceField } from './types'
...
export function FieldSettings({ field, onUpdateField }: FieldSettingsProps) {
  ...
  const handleOptionAdd = () => {
    if (field.type === 'radio' || field.type === 'select') {
      ...
      handleUpdate({ options: [...((field as ChoiceField).options || []), newOption] })
    }
  }
  ...
  const handleOptionDelete = (optionId: string) => {
    if (field.type === 'radio' || field.type === 'select') {
      const updatedOptions = (field as ChoiceField).options?.filter(option => option.id !== optionId)
      handleUpdate({ options: updatedOptions })
    }
  }
                ...
                <CardContent className="space-y-4">
                  ...
                  {(field.type === 'radio' || field.type === 'select') && (
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        {(field as ChoiceField).options?.map((option: Option, index: number) => (
                          <div key={option.id} className="flex items-center space-x-2">
                            <Input
                              value={option.label}
                              onChange={(e) =>
                                handleOptionUpdate(option.id, { label: e.target.value })
                              }
                            ...
                            <Input
                              value={option.value}
                              onChange={(e) =>
                                handleOptionUpdate(option.id, { value: e.target.value })
                              }
                              placeholder="value"
                              className="w-20"
                            />
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleUpdate({
                          appearance: { ...field.appearance, placeholder: e.target.value },
                        })
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleUpdate({
                          appearance: { ...field.appearance, helpText: e.target.value },
                        })
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleUpdate({
                          appearance: {
                            ...field.appearance,
                            width: e.target.value as 'auto' | 'full' | '1/2' | '1/3',
                          },
                        })
                      }
