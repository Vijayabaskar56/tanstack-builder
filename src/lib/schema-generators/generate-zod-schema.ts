...
const sanitizeFieldName = (name: string | undefined): string => {
	if (!name) return "";
...
const processFormElements = (
	...
	for (const element of elements) {
		...
		if (isFormArray(element)) {
			if (!element.name || element.name.trim() === "") continue;
...
const processArrayFields = (
	...
	for (const field of fields) {
		...
		if (isFormArray(field)) {
			...
			if (isStatic(field.fieldType) || !field.name || field.name.trim() === "")
				continue;
